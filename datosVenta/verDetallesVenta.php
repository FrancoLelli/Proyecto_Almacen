...
<body>
    ...
    <main>
        <div>
            <h1 class="tituloPagina">DETALLE VENTA CON ID: <?php echo $_GET['id'] ?></h1>
        </div>
        <section class="panelProv">
            ...
            <table class="table table-striped">
                ...
                <tbody>
                    <?php
                        $id = $_GET['id'];
                        $arrayConsulta = $db->query("SELECT (cliente.Cli_nombre) as 'Nombre', (cliente.Cli_apellido) as 'Apellido', (prod.Prod_nombre) as 'Producto', (prod_venta1.ProdVent_cantidad) as 'Cantidad', (prod_venta1.precioVenta) as 'Precio' FROM prod_venta1, prod, cliente, venta1 WHERE prod.Prod_id = prod_venta1.ProdVent_prod AND cliente.Cli_id = venta1.Vent_id_cliente AND venta1.Vent_id = prod_venta1.ProdVent_venta AND prod_venta1.ProdVent_venta = '$id'");
                        $arrayConsulta2 = 
                        $consulta = $arrayConsulta->fetch_array();

                        while($consulta != null){
                            echo "<tr> <td> <b>".$consulta["Nombre"]."</b></td>";
                            echo "<td> ".$consulta["Apellido"]."</td>";
                            echo "<td> ".$consulta["Producto"]."</td>";
                            echo "<td> ".$consulta["Cantidad"]."</td>";
                            echo "<td> $".$consulta["Precio"]."</td></tr>";
            ...
            <a href="./verTodasVentas.php"><button class='btn btn-secondary'>Volver</button></a>
