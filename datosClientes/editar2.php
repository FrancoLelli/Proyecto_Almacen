    ...
    <main>
        ...
        <section class="panelProv">
            <div class="formProv">
                <?php
                    $id= $_GET['id'];
                    $queryDatos = $db->query("SELECT (cliente.Cli_nombre) as 'Nombre', (cliente.Cli_apellido) as 'Apellido', (loc.Loc_nombre) as 'Ciudad', (loc.loc_id) as 'id_loc', Cli_DNI as 'DNI', estadoReal FROM cliente,loc WHERE loc.Loc_id = cliente.Cli_id_loc AND Cli_id = '$id'");
                    $arrayUsuario = $queryDatos->fetch_array();
                ?>
                <form class="form" id="formulario" method="POST">
                    <div class="tituloForm"><h2>Editar Cliente con ID: <?php echo $_GET['id'] ?></h2></div>
                    <hr>
                    <div class="divForm">
                        <label for="name">Nombre:</label>
                        <input type="text" name="name" placeholder="Ingresar nombre" id="nombreCli" value="<?php echo $arrayUsuario['Nombre'] ;?>">
                    </div>
                    <div class="divForm">
                        <label for="apellido">Apellido:</label>
                        <input type="text" name="apellido" placeholder="Ingresar apellido" id="apellidoCli" value="<?php echo $arrayUsuario['Apellido'] ;?>">
                    </div>
                    <div class="divForm">
                        <label for="dni">DNI:</label>
                        <input type="number" name="dni" placeholder="Ingresar dni" id="dniCli" value="<?php echo $arrayUsuario['DNI'] ;?>">
                    </div>
                    <div class="divForm">
                    ...
                    <div class="divForm">
                        ...
                        <button type="reset" class="btn btn-secondary"><a style="text-decoration:none; color:white"class="enlacesBtn" href="../clientes.php">Volver</a></button>
                    </div>
                </form>
                <?php 
                    if(!isset($_POST['actualizar'])){
                        ...
                        $actualizarDatos = $db->query("UPDATE cliente SET Cli_nombre = '$nom', Cli_apellido = '$apellido', Cli_DNI = '$dni', Cli_id_loc = '$loc', estadoReal = '$estado' WHERE Cli_id = '$id'");
